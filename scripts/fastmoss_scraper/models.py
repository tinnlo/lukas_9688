"""Data models for FastMoss scraper - independent implementation."""
...
@dataclass
class SalesData:
    """Sales data from 数据总览 section."""
    date_range: str  # '28day' for FastMoss
...
@dataclass
class VideoAnalysis:
    """Video analysis metrics from 商品关联视频 tab."""
...
@dataclass
class VideoData:
    """Individual video data from FastMoss."""
    rank: int
    title: str
    creator_username: str
    creator_followers: Optional[int] = None
    publish_date: Optional[str] = None
    estimated_sales: Optional[int] = None  # 销量(近28天)
    estimated_revenue: Optional[str] = None  # 销售额(近28天)
    total_views: Optional[int] = None  # 播放量
    likes: Optional[int] = None  # 点赞数
    comments: Optional[int] = None  # 评论数
    engagement_rate: Optional[str] = None  # 互动率
...
@dataclass
class ProductData:
    """Complete product data structure for FastMoss."""
    product_id: str
    scraped_at: str
...
@dataclass
class ScraperConfig:
    """Configuration for FastMoss scraper."""
    headless: bool = True
    timeout: int = 30000
    max_retries: int = 3
