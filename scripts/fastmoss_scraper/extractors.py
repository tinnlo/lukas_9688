...
class DataExtractor:
    ...
    async def extract_top_videos(self, limit: int = 5) -> List[VideoData]:
        ...
        try:
            ...
            video_data_list = await self.page.evaluate('''(limit) => {
                const videos = [];
                ...
                for (const table of tables) {
                    ...
                    for (let i = 0; i < rows.length && videos.length < limit; i++) {
                        ...
                        try {
                            ...
                            const sales = salesMatch ? parseInt(salesMatch[0]) : null;
                            ...
                            const views = viewsMatch ? parseInt(viewsMatch[0]) : null;

                            // Column 5: Likes (点赞数)
                            const likesText = cells[4]?.textContent?.trim() || '0';
                            const likesMatch = likesText.match(/\\d+/);
                            const likes = likesMatch ? parseInt(likesMatch[0]) : null;

                            // Column 6: Comments (评论数)
                            const commentsText = cells[5]?.textContent?.trim() || '0';
                            const commentsMatch = commentsText.match(/\\d+/);
                            const comments = commentsMatch ? parseInt(commentsMatch[0]) : null;
            ...
            for video_dict in video_data_list:
                try:
                    ...
                    if fastmoss_url:
                        try:
                            logger.debug(f"Navigating to video detail page: {fastmoss_url}")
                            await self.page.goto(fastmoss_url, wait_until="domcontentloaded", timeout=15000)
                            await asyncio.sleep(2)
                            ...
                            if tiktok_url:
                                ...
                                logger.warning(f"No TikTok URL found on video detail page")

                        except Exception as e:
                            logger.warning(f"Failed to navigate to video detail page: {e}")
            ...
            logger.success(f"Extracted {len(videos)} videos from table with TikTok URLs")
