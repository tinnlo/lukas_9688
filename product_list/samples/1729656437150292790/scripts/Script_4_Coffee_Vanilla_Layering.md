---
cover: ""
caption: >-
  Wusstest du, dass du Parfüms mischen kannst? Kaffee-Vanille-Kombi ist genial! LASTAR Layering-Hack für deinen einzigartigen Duft #parfüm #layering #vanille #kaffee #beautyhack #ParfümLayering #BeautyHack #VanilleDuft #KaffeeNoten #TikTokShop
published: 2026-02-02
duration: "00:42"
sales:
  - yes
link: ""
tags:
  - "#ParfümLayering"
  - "#BeautyHack"
  - "#VanilleDuft"
  - "#KaffeeNoten"
  - "#TikTokShop"
product: "LASTAR Vanille Parfüm Geschenkbox"
source_notes:
  - "product_list/samples/1729656437150292790/ref_video/video_synthesis.md"
  - "product_list/samples/1729656437150292790/product_images/image_analysis.md"
  - "product_list/samples/1729656437150292790/tabcut_data.json"
---

## Scripts

**Structure: Educational Layering Expert**

1. **Hook (0-3s):** "You know you can MIX perfumes?"
2. **Setup (3-10s):** Introduce the 2-bottle concept - coffee vs. rose
3. **Demo (10-28s):** Show layering technique, explain scent notes
4. **Proof (28-37s):** Why it works + affordability angle (2 bottles = endless combos)
5. **CTA (37-42s):** Encourage experimentation + shop link

---

## Visual Strategy

| Seconds | Visual | Purpose |
|:--------|:-------|:--------|
| 00-03 | Creator holding both bottles, questioning expression | Hook: "Did you know?" curiosity |
| 03-08 | Close-up: Amber bottle + text "Kaffee, Vanille, Holz" | Education: warm notes |
| 08-13 | Close-up: Pink bottle + text "Rose, Jasmin, Vanille" | Education: floral notes |
| 13-18 | Spray amber bottle on LEFT wrist (slow-mo) | Demo: layering step 1 |
| 18-23 | Spray pink bottle on RIGHT wrist (slow-mo) | Demo: layering step 2 |
| 23-28 | Wrists together, creator smells both | Proof: scent synergy |
| 28-33 | Split screen: coffee beans + rose petals | Sensory visualization |
| 33-37 | Text overlay: "2 Flaschen = 10+ Kombinationen" | Value: endless customization |
| 37-42 | Product hero + yellow basket CTA | Conversion |

---

## Voiceover

### DE (ElevenLabs Prompt | 42s)

> with ElevenLabs v3 (alpha) grammar

[curious] Wusstest du, dass du Parfüms mischen kannst?
[bright] Scent Layering ist der neue Trend.

[neutral] Dieses Set von LASTAR ist perfekt dafür.
[educational] Bernsteinfarbene Flasche: Kaffee, Vanille, Holz.
[educational] Rosa Flasche: Rose, Jasmin, Vanille.

[matter-of-fact] Beide haben Vanille. Das ist der Schlüssel.
[confident] Die Noten verschmelzen dadurch harmonisch.

[neutral] Ich sprühe "For One Night" auf das eine Handgelenk.
[neutral] Dann "Forever Love" auf das andere.
[playful] Und jetzt? Beide zusammen.

[impressed] Oh. Das riecht wie ein komplett neuer Duft.
[enthusiastic] Warm, blumig, ein bisschen mysteriös.

[neutral] Für unter 20 Euro bekommst du zwei Düfte.
[confident] Aber eigentlich zehn verschiedene Kombinationen.

[encouraging] Probier's aus. Experimentiere.
[bright] Link im Profil. Hol dir das Set.

---

### ZH (中文翻译 | 42s)

[curious] 你知道香水可以混合吗？
[bright] 香水叠加是新趋势。

[neutral] LASTAR 的这个套装非常完美。
[educational] 琥珀色瓶：咖啡、香草、木质。
[educational] 粉色瓶：玫瑰、茉莉、香草。

[matter-of-fact] 两者都有香草。这是关键。
[confident] 所以香调能和谐融合。

[neutral] 我把"For One Night"喷在一侧手腕。
[neutral] 然后把"Forever Love"喷在另一侧。
[playful] 现在呢？两个一起闻。

[impressed] 哦。这闻起来像全新的香水。
[enthusiastic] 温暖、花香、有点神秘。

[neutral] 不到 20 欧元你能得到两种香水。
[confident] 但实际上是十种不同的组合。

[encouraging] 试试看。实验一下。
[bright] 简介里的链接。快买这个套装。
