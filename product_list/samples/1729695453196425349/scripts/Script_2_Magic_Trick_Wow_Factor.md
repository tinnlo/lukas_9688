---
cover: ""
caption: >-
  Wetten du wusstest nicht dass dein Handy DAS kann 😱 PAXA macht aus jedem Handy einen krassen Lautsprecher. Keine App nötig probiers #tiktokshop #paxa #magic #gadget #viral #MagicGadget #PAXA #PhoneHack #TechMagie #TikTokShop
published: 2026-01-31
duration: "00:30"
sales:
  - yes
link: ""
tags:
  - "#MagicGadget"
  - "#PAXA"
  - "#PhoneHack"
  - "#TechMagie"
  - "#TikTokShop"
product: "PAXA-C-280 Lautsprecherständer"
source_notes:
  - "product_list/samples/1729695453196425349/ref_video/video_synthesis.md"
  - "product_list/samples/1729695453196425349/product_images/image_analysis.md"
  - "product_list/samples/1729695453196425349/tabcut_data.json"
---
 
## Scripts

**Structure: Magic Trick Entertainment**

1. **Hook (0-3s):** "I bet you didn't know your phone could do THIS"
2. **Demo (3-15s):** Rapid sequence - drop phone → loud, lift → silence, repeat 3x
3. **Reveal (15-25s):** "How does it work? Induction technology"
4. **Feature (25-28s):** Works with every phone, no app needed
5. **CTA (28-30s):** Grab this magic block now

---

## Visual Strategy

| Seconds | Visual | Purpose |
|:--------|:-------|:--------|
| 00-03 | Creator looks at camera with mischievous smile, "Achte auf den Moment..." | Hook: Curiosity/anticipation |
| 03-06 | Phone on table playing quiet music | Setup: Before state |
| 06-09 | Hand drops phone on PAXA → Music EXPLODES (camera shake effect on beat) | Magic: Wow moment #1 |
| 09-11 | Hand lifts phone → Silence | Magic: Contrast |
| 11-13 | Hand drops phone again → Music LOUD | Magic: Repetition proves it |
| 13-16 | Creator's face: "Wie bitte?!" expression, looks at camera | Reaction: Shared amazement |
| 16-19 | Close-up of speaker mesh vibrating (with rice on top to visualize bass) | Proof: Real technology |
| 19-22 | Text overlay: "Induktions-Technologie" with science-y graphics | Education: Credibility |
| 22-25 | Show iPhone, Samsung, Huawei logos - "Funktioniert mit allen" | Trust: Universal compatibility |
| 25-28 | Show phone with thick case working on PAXA | Objection Handling: Case compatible |
| 28-30 | Point down, "Link ist hier" | Conversion |

---

## On-Screen Text

**Strategy:** Curiosity Question — triggers scroll-stop curiosity for muted viewers, matching the counter-intuitive "Wetten du wusstest nicht" verbal hook

| Timing | DE Text                               | ZH Text             | Placement     |
| :----- | :------------------------------------ | :------------------ | :------------ |
| 0–3s   | **"Wetten, dein Handy kann DAS? 😱"** | **"打赌你的手机能做这个？😱"** | Center        |
| 16–19s | Induktions-Technologie 🔊             | 感应技术 🔊             | Top           |
| 22–25s | Funktioniert mit JEDEM Handy          | 所有手机通用              | Center-bottom |

---

## Voiceover

### DE (ElevenLabs Prompt | 30s)

> with ElevenLabs v3 (alpha) grammar

[mischievous] Wetten, du wusstest nicht, dass dein Handy DAS kann?

[whisper] Achte genau hin.

[neutral] Musik ist leise. Handy drauf. BOOM.
[excited] Wieder runter. Still.
[neutral] Drauf. Laut.

[neutral] Wie geht das? Magie?
[curious] Nein. Induktion.

[neutral] PAXA nutzt Schall-Übertragung. Kein Bluetooth.
[educational] Funktioniert mit jedem Handy.

[matter-of-fact] iPhone? Ja. Samsung? Ja. Mit Hülle? Auch ja.

[neutral] Das Ding ist der Wahnsinn.
[cheerfully] Schnapp's dir. Jetzt.


---

---

### ZH (中文翻译 | 30s)

[mischievous] 打赌你不知道你的手机能做到这个？

[whisper] 仔细看。

[neutral] 音乐很小声。放上去。轰。
[excited] 拿下来。安静。
[neutral] 放上去。大声。

[neutral] 怎么做到的？魔法？
[curious] 不。感应。

[neutral] PAXA 使用声音传输。无蓝牙。
[educational] 适用于所有手机。

[matter-of-fact] iPhone？是的。三星？是的。带壳？也是的。

[neutral] 这东西太疯狂了。
[cheerfully] 快抢。现在。
